...
{
    public partial class ReturnView : WMS.App_Code.BasePage
    {
        ...
        protected void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            if (StartToK3 > 0)
            {
                ...
                try
                {
                    ...
                    paras[2] = new DataParameter("@ReChk", blnReChk);

                    bll.ExecNonQueryTran("WMS.SPCheckReturn", paras);

                    AddOperateLog("销售退货单审核", this.txtID.Text);
                    DataTable dt = bll.FillDataTable("WMS.SelectReturn", new DataParameter[] { new DataParameter("{0}", string.Format("{0}='{1}' and Flag={2}", PrimaryKey, this.txtID.Text, Flag)) });
                    BindData(dt);
                }
                catch (Exception ex)
                {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            AddOperateLog("销售退货单", "删除单号：" + strID);
