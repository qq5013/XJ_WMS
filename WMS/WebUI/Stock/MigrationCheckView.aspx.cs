...
{
    public partial class MigrationCheckView : WMS.App_Code.BasePage
    {
        ...
        private string Filter = "State>=3 and Flag=1";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                DataTable dt = bll.FillDataTable("WMS.SelectMigration", new DataParameter[] { new DataParameter("{0}", string.Format("{0}='{1}' and State>=3 and Flag={2}", PrimaryKey, ID, Flag)) });
                BindData(dt);
        ...
        {
            DataTable dt = bll.FillDataTable("WMS.SelectMigrationDetail", new DataParameter[] { new DataParameter("{0}", string.Format("BillID='{0}' and Flag={1}", this.txtID.Text, Flag)) });
        ...
        protected void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            if (StartToK3 > 0)
            {
                DataTable dtK3Main = bll.FillDataTable("WebService.SelectBackToK3Main", new DataParameter[] { new DataParameter("@BillID",  this.txtID.Text) });
                DataTable dtK3Sub = bll.FillDataTable("WebService.SelectBackToK3Sub", new DataParameter[] { new DataParameter("@BillID", this.txtID.Text) });
                ...
                try
                {
                    ...
                    bll.ExecNonQueryTran("WMS.SPCheckMigration", paras);
                    AddOperateLog("移库单审核", this.txtID.Text);
                    DataTable dt = bll.FillDataTable("WMS.SelectMigration", new DataParameter[] { new DataParameter("{0}", string.Format("{0}='{1}' and Flag={2}", PrimaryKey, this.txtID.Text, Flag)) });
                    BindData(dt);
                }
                catch (Exception ex)
                {
