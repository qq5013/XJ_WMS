...
{
    public partial class InStockReturnView : WMS.App_Code.BasePage
    {
        ...
        protected void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            if (StartToK3 > 0)
            {
                ...
                try
                {
                    ...
                    bll.ExecNonQueryTran("WMS.spCheckInstockReturn", paras);

                    AddOperateLog("入库退货单审核", this.txtID.Text);
                    DataTable dt = bll.FillDataTable("WMS.SelectInStockReturn", new DataParameter[] { new DataParameter("{0}", string.Format("{0}='{1}' and Flag={2}", PrimaryKey, this.txtID.Text, Flag)) });
                    BindData(dt);
                }
                catch (Exception ex)
                {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            AddOperateLog("入库退货单", "删除单号：" + strID);
