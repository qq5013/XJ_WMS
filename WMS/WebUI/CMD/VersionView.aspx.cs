...
{
    public partial class VersionView : WMS.App_Code.BasePage
    {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            //int Count = bll.GetRowCount(TableName, string.Format("COLOR_CODE='{0}'", this.txtID.Text.Trim()));
            //if (Count > 0)
            //{
            //    WMS.App_Code.JScript.Instance.ShowMessage(this.Page, "该规格编码还被其它单据使用，请调整后再删除！");
            //    return;
            //}
            bll.ExecNonQuery("Cmd.DeleteVersion", new DataParameter[] { new DataParameter("{0}", strID) });
